(function() {
  ...
  window.submitManagerOrder = async function(formId, msgId) {
    ...
    try {
      ...
      details._created_from = "manager";

      /* Canonical manager order RPC */
      const { data: rows, error: oe } = await supabaseClient.rpc("create_gd_order", {
        ...
        p_requested_channel: "manager",
        p_order_number: orderNum,
      });
      if (oe) throw oe;
      const inserted = Array.isArray(rows) ? rows[0] : rows;
      if (!inserted?.id) throw new Error("Պատվերը չստեղծվեց");
      const savedOrderNumber = inserted.order_number || orderNum;

      showMsg(msgEl, "✅ Պատվեր " + savedOrderNumber + " գրանցված է", "success");
      if (typeof toast === "function") toast("Պատվեր " + savedOrderNumber + " գրանցված է ✓", "success");
