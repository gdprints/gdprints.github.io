...
(async function init(){
  ...
  subscribeToOrders((payload) => {
    if (payload.eventType === "UPDATE" && payload.new?.created_by_manager_id === MY_PROFILE.id && payload.new?.created_by_type === "manager" && payload.new?.source_channel === "manager"){
...
function computeEarning(order){
  if (order.created_by_type !== "manager" || (order.source_channel && order.source_channel !== "manager")) return 0;
...
async function loadMyOrders(){
  const { data, error } = await supabaseClient
    .from("orders")
    .select("*")
    .eq("created_by_type", "manager")
    .eq("source_channel", "manager")
...
async function loadOrderExtraDetails(orderId){
  ...
  el.innerHTML = detailRows.map(({label,value}) => `
    <div style="display:flex; justify-content:space-between; gap:14px; padding:8px 0; border-bottom:1px solid var(--border); font-size:13.5px;">
      <span style="color:var(--text-muted);">${GDOrderDetails.escapeHtml(label)}</span>
