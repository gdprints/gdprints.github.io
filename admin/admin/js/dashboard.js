...
(async function init(){
  try {
    ...
    setTextIfPresent("user-name", auth.profile.full_name || auth.session.user.email.split("@")[0]);
    setTextIfPresent("user-avatar", initials(auth.profile.full_name || auth.session.user.email));
    setTextIfPresent("today-date", new Date().toLocaleDateString("hy-AM", { weekday: "long", day: "numeric", month: "long" }));

    // Bind controls BEFORE network/data loading. A failed optional widget must never
    // make the whole Admin dashboard feel "dead".
    ...
    subscribeToOrders((payload) => {
      ...
      loadOrders().catch(err => console.error("Realtime order refresh failed", err));
    });

    await loadEverything();
  } catch (error) {
    console.error("Admin dashboard init failed", error);
    toast("Dashboard-ի բեռնումը մասամբ ձախողվեց։ Կառավարման կոճակները շարունակում են աշխատել։", "error");
  }
})();
...
function bindDashboardControls(){
  document.getElementById("refresh-btn")?.addEventListener("click", () => {
    loadEverything().catch(err => console.error("Refresh failed", err));
  });
  document.getElementById("save-order-btn")?.addEventListener("click", saveOrder);
  document.getElementById("delete-order-btn")?.addEventListener("click", deleteOrderPermanently);
  ...
  if (uploadZone) initUploadZone(uploadZone, handleFileUpload);
}

async function loadEverything(){
  const results = await Promise.allSettled([loadOrders(), loadPartnerBadge(), loadManagerCount()]);
  results.forEach((result, index) => {
    if (result.status === "rejected") {
      console.error(["loadOrders", "loadPartnerBadge", "loadManagerCount"][index] + " failed", result.reason);
    }
  });
  ...
  try {
    ...
    console.error("loadAttention failed", error);
  }
}
...
async function loadPartnerBadge(){
  ...
  // The unified sidebar no longer renders this badge on every page.
  // Skip the query as well when there is nowhere to display it.
  if (!badge) return;
...
async function loadManagerCount(){
  const target = document.getElementById("stat-managers");
  if (!target) return;
...
function renderOrdersTable(){
  ...
  body.innerHTML = list.map(o => `
    <tr>
      ...
      <td style="cursor:pointer;" onclick="openOrderDrawer('${o.id}')">${(o.created_by_type === "manager" && (!o.source_channel || o.source_channel === "manager") && o.profiles?.full_name) ? "👤 " + o.profiles.full_name : (o.source_channel === "customer_app" ? "📱 Customer App" : "🌐 Կայք")}</td>
      <td>
...
async function openOrderDrawer(orderId){
  ...
  if (order.created_by_type === "manager" && (!order.source_channel || order.source_channel === "manager") && order.profiles?.full_name){
    managerWrap.style.display = "block";
    document.getElementById("f-manager").value = order.profiles.full_name;
  } else {
    managerWrap.style.display = "block";
    document.getElementById("f-manager").value = order.source_channel === "customer_app" ? "Customer App" : "Կայքից (հաճախորդ)";
  }
...
async function deleteOrderPermanently(){
  if(!ACTIVE_ORDER)return;
  const order=ACTIVE_ORDER;
  const typed=prompt(`Պատվերը ՎԵՐՋՆԱԿԱՆ ջնջելու համար գրեք պատվերի համարը՝\n${order.order_number}\n\nԿջնջվեն պատվերը, մանրամասները, պատմությունը, ծանուցումները, դիզայնի հաստատումները և կցված ֆայլերը։`);
  if(typed===null)return;
  if(String(typed).trim()!==String(order.order_number).trim()){
    toast("Պատվերի համարը չի համընկնում․ ջնջումը չեղարկվեց","error");return;
  }
  if(!confirm(`Վերջնական հաստատո՞ւմ եք ${order.order_number} պատվերի ջնջումը։ Այս գործողությունը հետարկել հնարավոր չէ։`))return;
  ...
  try{
    const [{data:files,error:fileErr},{data:proofs,error:proofErr}]=await Promise.all([
      supabaseClient.from("order_files").select("id,file_name,file_url,storage_path").eq("order_id",order.id),
      supabaseClient.from("order_design_proofs").select("id,file_name,storage_path").eq("order_id",order.id)
    ]);
    if(fileErr)throw new Error(`Ֆայլերի տվյալներ: ${fileErr.message}`);
    if(proofErr)throw new Error(`Դիզայնի ֆայլեր: ${proofErr.message}`);
    const privatePaths=new Set(),legacyPublicPaths=new Set();
    (files||[]).forEach(f=>{if(f.storage_path)privatePaths.add(f.storage_path);const p=storagePathFromPublicUrl(f.file_url,"order-files");if(p)legacyPublicPaths.add(p);});
    (proofs||[]).forEach(p=>{if(p.storage_path)privatePaths.add(p.storage_path);});
    if(privatePaths.size){const {error}=await supabaseClient.storage.from("customer-order-files").remove([...privatePaths]);if(error)throw new Error(`customer-order-files Storage: ${error.message}`);}
    if(legacyPublicPaths.size){const {error}=await supabaseClient.storage.from("order-files").remove([...legacyPublicPaths]);if(error)throw new Error(`order-files Storage: ${error.message}`);}
    const {data:result,error:dbError}=await supabaseClient.rpc("admin_delete_order_permanently",{p_order_id:order.id});
    if(dbError)throw new Error(`Database: ${dbError.message}`);
    if(!result?.deleted)throw new Error("Պատվերի գրառումը չի ջնջվել");
    try{await logActivity(`Վերջնական ջնջեց պատվերը՝ ${order.order_number}`,"orders",order.id);}catch(_){}
    const no=order.order_number;ACTIVE_ORDER=null;closeDrawer();await loadOrders();await loadAttention();
    toast(`${no} պատվերը վերջնական ջնջված է`,"success");
  }catch(e){
    console.error("deleteOrderPermanently failed:",e);
    toast(`Պատվերը չհաջողվեց ջնջել${e?.message?": "+e.message:""}`,"error");
  }finally{if(btn){btn.disabled=false;btn.textContent="Ջնջել պատվերը";}}
}
...
async function loadOrderExtraDetails(orderId){
  ...
  const detailRows = window.GDOrderDetails
    ? GDOrderDetails.entries(data.details, { lang: "hy" })
    : Object.entries(data.details).filter(([k,v]) => v !== "" && v != null && !String(k).startsWith("_")).map(([k,v]) => ({ label:k, value:String(v) }));
  el.innerHTML = detailRows.map(({label,value}) => `
    <div style="display:flex; justify-content:space-between; gap:14px; padding:8px 0; border-bottom:1px solid var(--border); font-size:13.5px;">
      <span style="color:var(--text-muted); flex-shrink:0;">${GDOrderDetails.escapeHtml(label)}</span>
      <span style="text-align:right; font-weight:500; word-break:break-word;">${GDOrderDetails.escapeHtml(value)}</span>
    </div>
  `).join("");
}
