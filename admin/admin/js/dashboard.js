...
async function deleteOrderFile(fileId){
  ...
  try{
    ...
    // 2. Delete metadata using the Admin-only SECURITY DEFINER RPC.
    const { data:deleted, error:rpcError } = await supabaseClient
      .rpc('admin_delete_order_file_record', { p_file_id: fileId });
    if (rpcError) throw new Error(`Database: ${rpcError.message}`);
    if (deleted !== true) throw new Error('Տվյալների բազայի գրառումը չի ջնջվել');
