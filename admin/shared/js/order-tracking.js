...
async function loadTrackingExtraDetails(orderId){
  ...
  const detailRows = window.GDOrderDetails
    ? GDOrderDetails.entries(data.details)
    : Object.entries(data.details).filter(([k,v]) => v !== "" && v != null && !String(k).startsWith("_")).map(([k,v]) => ({ label:k, value:String(v) }));
  el.innerHTML = detailRows
    .map(({label,value}) => `<div class="gdt-row"><span>${GDOrderDetails.escapeHtml(label)}</span><span>${GDOrderDetails.escapeHtml(value)}</span></div>`).join("");
}
