...
function isManagerOrder(o){
  if(!o || !o.created_by_manager_id || o.created_by_type!=='manager') return false;
  return o.source_channel==='manager';
}
function renderAnalytics(){
  ...
  const earnings=valid.filter(isManagerOrder).reduce((s,o)=>s+earningFor(o, role==='manager' ? ANALYTICS_PROFILE?.commission_percent : profilePct(o.created_by_manager_id)),0);
...
function buildOrderRows(rows,role){
  return rows.map(o=>{
    ...
    return {
      ...
      manager:isManagerOrder(o)?(role==='manager'?(ANALYTICS_PROFILE?.full_name||''):managerName(o.created_by_manager_id)):'',
      earning:isManagerOrder(o)?earningFor(o,pct):0, note:o.notes||o.comment||o.description||''
    };
  });
}
...
async function buildExcelReport(rows,label,role,filename,title){
  ...
  if(role==='admin'){
    const mm={};valid.filter(isManagerOrder).forEach(o=>{const id=o.created_by_manager_id,p=ANALYTICS_PROFILES.find(x=>x.id===id);mm[id]??={name:p?.full_name||'Մենեջեր',n:0,r:0,e:0};mm[id].n++;mm[id].r+=Number(o.total_amount)||0;mm[id].e+=earningFor(o,p?.commission_percent)});
