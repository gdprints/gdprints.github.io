...
async function loadOrderById(id){const {data,error}=await supabaseClient.from('orders').select('*').eq('id',id).maybeSingle();if(error)throw error;return data}
...
async function markCustomerNotificationRead(id){if(!id)return;await supabaseClient.from('customer_app_notifications').update({is_read:true}).eq('id',id)}
...
async function maybeOfferCustomerPush(){
  try{
    ...
    x.innerHTML='<div style="width:min(430px,100%);padding:26px;border-radius:24px;background:#111318;border:1px solid rgba(255,38,56,.28);color:#fff;box-shadow:0 30px 80px rgba(0,0,0,.45)"><div style="font-size:11px;font-weight:800;letter-spacing:.12em;color:#ff2638;margin-bottom:8px">GDPRINT NOTIFICATIONS</div><h2 style="margin:0 0 12px;color:#fff">Միացնել իրական ծանուցումները</h2><p style="color:#c4c7cd;line-height:1.6;font-size:14px">Ստացեք պատվերի կարգավիճակի, պատրաստ լինելու, դիզայնի և առաքման մասին ծանուցումներ հեռախոսի վրա՝ նույնիսկ երբ հավելվածը փակ է։</p><div style="display:flex;gap:10px;margin-top:20px"><button id="gdPushYes" style="flex:1;min-height:48px;border:0;border-radius:13px;background:linear-gradient(135deg,#ff3144,#d90b20);font-weight:800">Միացնել</button><button id="gdPushLater" style="min-height:48px;padding:0 18px;border-radius:13px;border:1px solid #333;background:#1b1d21;color:#fff">Հետո</button></div><div id="gdPushErr" style="display:none;margin-top:12px;color:#ffb4b4;font-size:12px"></div></div>';
...
/* === v7.5 customer conversations + account security === */
async function loadCustomerMessageThreads(){
  const {data,error}=await supabaseClient.rpc('customer_message_threads');
  if(error)throw error; return Array.isArray(data)?data:(data||[]);
}
async function loadCustomerOrderMessages(orderId){
  const {data,error}=await supabaseClient.rpc('customer_order_message_bundle',{p_order_id:orderId});
  if(error)throw error; return data||{order:null,messages:[]};
}
async function sendCustomerOrderMessage(orderId,message){
  const {data,error}=await supabaseClient.rpc('customer_send_order_message',{p_order_id:orderId,p_message:String(message||'').trim()});
  if(error)throw error; return data;
}
async function repeatCustomerOrder(orderId){
  const {data,error}=await supabaseClient.rpc('repeat_customer_order',{p_order_id:orderId});
  if(error)throw error; return data && (Array.isArray(data)?data[0]:data);
}
async function requestPasswordReset(email){
  const redirectTo=new URL('reset-password.html',location.href).href;
  const {error}=await supabaseClient.auth.resetPasswordForEmail(String(email||'').trim(),{redirectTo});
  if(error)throw error; return true;
}
async function updateCustomerPassword(password){
  if(String(password||'').length<8)throw new Error('Գաղտնաբառը պետք է լինի առնվազն 8 նիշ');
