...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!RESEND_API_KEY) throw new Error("RESEND_API_KEY secret-ը սահմանված չէ");
    ...
    const { data: actor } = await adminClient
      ...
      .single();
    if (actor?.role !== "admin") throw new Error("Այս գործողությունը հասանելի է միայն ադմինին");
    ...
    if (manager.approval_status !== action) throw new Error("Մենեջերի կարգավիճակը դեռ չի պահպանվել");
    ...
    if (!recipientEmail) throw new Error("Մենեջերի email-ը չի գտնվել");
